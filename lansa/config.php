...
function ubahPass ($data) {
    ...
    $pw = mysqli_fetch_assoc(mysqli_query($conn, "SELECT * FROM users WHERE id_user = $id_user"))['password'];
    $pwbaru = md5($data['upassbaru']);

    if (md5($data['passwordlama']) == $pw) {

        $query = "UPDATE users SET password = '$pwbaru' WHERE id_user = $id_user";
...
function find($keyword) {
    $query = "SELECT * FROM pesanan
                ...
                kode_booking LIKE '$keyword%' OR
                id_jurusan LIKE '$keyword%' OR
                id_user LIKE '$keyword%' OR
                tgl_berangkat LIKE '$keyword%' OR
                tgl_pesan LIKE '$keyword%' OR
                id_kursi LIKE '$keyword%' OR
                harga LIKE '$keyword%' 
            ";
    return query($query);
}
...
function getPesanan($id){
    global $conn;
    return mysqli_fetch_array(mysqli_query($conn,"SELECT * FROM pesanan WHERE id_pesanan=$id"));
}

function getJurusan($id){
    global $conn;
    return mysqli_fetch_array(mysqli_query($conn,"SELECT * FROM jurusan WHERE id_jurusan=$id"));
}

function getUser($id){
    global $conn;
    return mysqli_fetch_array(mysqli_query($conn,"SELECT * FROM users WHERE id_user=$id"));
}

function getKursi($id){
    global $conn;
    return mysqli_fetch_array(mysqli_query($conn,"SELECT * FROM kursi WHERE id_kursi=$id"));
}
...
function pesan($data) {
        ...
        $id_kursi = htmlspecialchars($data["id_kursi"]);
        $harga = htmlspecialchars($data["harga"]);
        //query insert data
       
       $query = "INSERT INTO pesanan
                VALUES
        ('', '$kode_booking','$id_jurusan', '$id_user', '$tgl_berangkat',  '$jemput', '$tgl_pesan', '$id_kursi', '$harga')";
...
function bayar($data){
    ...
    foreach ($data['kodepesan'] as $pesanan) {
        $query = "INSERT INTO payment
                    VALUES
                    (NULL, '$kode_payment','$pesanan', $id_user , '$tgl_payment',  'Belum', '$gambar')";
        
        mysqli_query($conn, $query);

        $query2 = "UPDATE pesanan SET bayar=1 WHERE id_pesanan=$pesanan";
...
function konfirmasibayar($id_pesanan){
    global $conn;
    mysqli_query($conn, "UPDATE payment SET status_bayar='Sudah' WHERE id_pesanan = $id_pesanan");
