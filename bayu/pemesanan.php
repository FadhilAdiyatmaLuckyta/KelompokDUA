...
require 'config.php';
	...
	$sql = "SELECT max(kode_booking) FROM pesan";
	$query = mysqli_query($sql) or die (mysqli_error());
 
	$kode_booking = mysql_fetch_array($query);
 
	if($kode_booking){
		$nilai = substr($kode_booking[0], 1);
		$kode = (int) $nilai;
...
if( isset($_POST["submit"]) ){
    ...
    if( pesan($_POST) > 0 ) {
       echo "
       Berhasil pesan, silahkan <a href='pilih_kursi.php'>pilih kursi</a> disini";
...
<html>
	...
	<body>
		...
		<form action="vesan.php" method="post">
		<table>

		<tr>
				<td>Kode Booking</td>
				<td>:</td>
				<td><input type="text" name="kode_booking" value="<?php echo $auto_kode;?>" readonly="readonly"></td>
		</tr>
